...
public class Card {
    ...
    public void setCharge(double amt, String desc) {
        ...
        if (amt <= 0) {
            actmsg = "Charge of " + c.format(amt) + " for " + desc + " declined - illegal amount.";
            writelog(actmsg);
        } else if ((this.BalDue + amt) > this.CLimit) {
            actmsg = "Charge of " + c.format(amt) + " for " + desc + " declined - over limit!";
            writelog(actmsg);
        } else {
            this.BalDue += amt;
            writestatus();
            if (this.errmsg.isEmpty()) {
                actmsg = "Charge of " + c.format(amt) + " for " + desc + " posted.";
                writelog(actmsg);
            }
        }
    } //end of setcharge
